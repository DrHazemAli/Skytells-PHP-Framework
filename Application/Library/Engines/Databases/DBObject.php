...
Class DBObject extends Model {
   ...
   **/
    /**
     * Working instance of SQLManager created earlier
     *
     * @var SQLManager
     */
    public $db;
    ...
    private function byId ($id, $fields = null) {
        $this->db->where (SQLManager::$prefix . $this->dbTable . '.' . $this->primaryKey, $id);
    ...
    private function join ($objectName, $key = null, $joinType = 'LEFT', $primaryKey = null) {
        ...
        if (!$key)
            $key = $objectName . "id";
        if (!$primaryKey)
            $primaryKey = SQLManager::$prefix . $joinObj->dbTable . "." . $joinObj->primaryKey;

        if (!strchr ($key, '.'))
            $joinStr = SQLManager::$prefix . $this->dbTable . ".{$key} = " . $primaryKey;
        else
            $joinStr = SQLManager::$prefix . "{$key} = " . $primaryKey;
     ...
     *
     * Provides magic access to private functions of the class and native public SQLManager functions
     *
     * @param string $method
     * @param mixed $arg
     *
