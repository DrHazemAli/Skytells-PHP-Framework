...
Class SQLManager extends Model
{
     ...
     *
     * @uses $db = SQLManager::getInstance();
     *
     * @return SQLManager Returns the current instance.
     */
     ...
     *
     * @uses $SQLManager->setQueryOption('name');
     *
     ...
     *
     * @uses $SQLManager->where('id', 7)->where('title', 'MyTitle');
     *
     ...
     *
     * @uses $SQLManager->orWhere('id', 7)->orWhere('title', 'MyTitle');
     *
     * @param string $whereProp  The name of the database field.
     * @param mixed  $whereValue The value of the database field.
     * @param string $operator Comparison operator. Default is =
     ...
     *
     * @uses $SQLManager->having('SUM(tags) > 10')
     *
     ...
     *
     * @uses $SQLManager->orHaving('SUM(tags) > 10')
     *
     * @param string $havingProp  The name of the database field.
     * @param mixed  $havingValue The value of the database field.
     * @param string $operator Comparison operator. Default is =
     ...
     *
     * @uses $SQLManager->join('table1', 'field1 <> field2', 'LEFT')
     *
     ...
     *
     * @uses $SQLManager->orderBy('id', 'desc')->orderBy('name', 'desc');
     *
	 ...
	 *
	 * @throws Exception
	 * @return SQLManager if succeeeded;
	 */
    ...
    /**
     * Method creates new SQLManager object for a subquery generation
     *
    ...
    /**
     * Method returns a copy of a SQLManager subquery object
     *
     * @return SQLManager new SQLManager object
     */
    ...
    /**
     * Get where and what function was called for query stored in SQLManager->trace
     *
    ...
    public function toCSV($array, $filename) {
      try {
        if (!isset($array)) { return "Callback function is missing out!"; }
        if (!isset($filename)) { return "Filename is missing"; }
        if (!is_array($array)){
          return "DB response or results must be an array!";
        }
        $f = fopen('php://temp', 'wt');
        $first = true;
